...
export class Repository<T> {
  ...
  async findById(key: Partial<T>): Promise<T | null> {
    ...
    const missing = primaryColumns.map((pc) => pc.propertyName).filter((prop) => !(prop in key));
    if (missing.length > 0) {
  ...
  async create(entity: Partial<T>): Promise<Partial<T>> {
    ...
    const requiredPrimaries = primaryColumns.filter((c) => !isAutoGeneratedType(c.type));
    const missing = requiredPrimaries
      .map((pc) => pc.propertyName)
      .filter((prop) => !(prop in entity));
    if (missing.length > 0) {
      throw new IncompletePrimaryKeyError(this.entity.name, missing);
    }
